...
class Blockchain():
    ...
    def instantiate_contract(self):
        ...
        if block['number']==0:
            ...
            tx_hash = Prescription.constructor().transact({'from': self.web3.eth.accounts[0], 'gasPrice': self.web3.eth.gas_price, 'gas': self.web3.eth.get_block('latest').gasLimit})
            ...
            hex_hash = self.web3.to_hex(block['transactions'][0])
            tx_hash = self.web3.eth.get_transaction_receipt(hex_hash)
    ...
    def insert_hash(self, hash_):
        ...
        if self.instantiate_contract():
            ...
            # except:
            #     return False
            success = self.contract.functions.add_hash(hash_).transact({'from': self.web3.eth.accounts[0], 'gasPrice': self.web3.eth.gas_price, 'gas': self.web3.eth.get_block('latest').gasLimit})    
            return success
